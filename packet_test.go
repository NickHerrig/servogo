...
func TestChecksumByte(t *testing.T) {
	tests := []struct {
		...
		{"four byte packet", []byte{0x02, 0x82, 0x80}, 0x84},
		{"five byte packet", []byte{0x01, 0x01, 0x01, 0x01}, 0x84},
		{"six byte packet", []byte{0x01, 0x01, 0x01, 0x01, 0x01}, 0x85},
		{"seven byte packet", []byte{0x01, 0x01, 0x01, 0x01, 0x01, 0x01}, 0x86},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
